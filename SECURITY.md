...
cesrview is a browser tool that parses untrusted input — you paste a CESR stream and it renders it. In scope: anything that turns pasted data into script execution, and anything that makes the rendering misrepresent what a stream actually says, since a viewer that lies is worse than no viewer.

It parses in the browser and sends nothing anywhere. If you find that untrue, that is a report.
