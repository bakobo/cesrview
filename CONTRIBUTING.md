...
Branch, push the branch, open a pull request against `main`. `build-and-test` must pass. Maintainers merge with a **merge commit or a rebase, never a squash** — the individual commits are the record, and collapsing them loses the per-commit history and the sign-off chain.
